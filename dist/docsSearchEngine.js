...
exports.DocsSearchEngine = void 0;
exports.formatDocsMetadataContext = formatDocsMetadataContext;
...
// Only instantiated in the retrieval worker. Corpus objects never cross its port.
class DocsSearchEngine {
    constructor(settings, logger, environment = { cwd: process.cwd(), serverDocs: process.env.CODEX_ELEMENT_SERVER_DOCS ?? "" }) {
        this.settings = settings;
        this.logger = logger;
        this.environment = environment;
        this.cacheByRoot = new Map();
        this.requestFingerprints = new Map();
        this.metrics = { cacheHits: 0, cacheMisses: 0, evictions: 0, loads: 0, lastLoadMs: 0 };
    }
    ...
    async buildPlannerInput() {
        ...
        return this.plannerInputFromCaches(caches);
    }
    plannerInputFromCaches(caches) {
    ...
    async overviewTool(options = {}) {
        ...
        const plannerInput = this.plannerInputFromCaches(caches);
        return {
            roots: summarizeCaches(caches),
    ...
    async load(root) {
        ...
        if (cached && fingerprint && cached.fingerprint?.value === fingerprint.value) {
            this.metrics.cacheHits += 1;
            this.cacheByRoot.delete(normalizedRoot);
            this.cacheByRoot.set(normalizedRoot, cached);
            this.requestFingerprints.set(normalizedRoot, fingerprint.value);
            return cached;
        }
        this.cacheByRoot.delete(normalizedRoot);
        this.metrics.cacheMisses += 1;
        const started = node_perf_hooks_1.performance.now();
        // Do not label a mixed read with a pre-read fingerprint. One retry is bounded.
        for (let attempt = 0; attempt < 2; attempt += 1) {
            const before = attempt === 0 ? fingerprint : (0, docsCorpusService_1.fingerprintDocsCorpora)(normalizedRoot);
            const loaded = await (0, docsCorpusService_1.loadDocsCorpora)(normalizedRoot, docsWorkerProtocol_1.DOCS_WORKER_LIMITS);
            const after = (0, docsCorpusService_1.fingerprintDocsCorpora)(normalizedRoot);
            if (!before || before.value !== after?.value) {
                continue;
            }
            const chars = loaded.fragments.reduce((total, fragment) => total + (0, docsCorpusService_1.docsFragmentChars)(fragment), 0);
            const cache = { root: normalizedRoot, fingerprint: after, loaded, chars };
            this.requestFingerprints.set(normalizedRoot, after.value);
            this.cacheByRoot.set(normalizedRoot, cache);
            while (this.cacheByRoot.size > docsWorkerProtocol_1.DOCS_WORKER_LIMITS.maxCacheRoots
                || this.getMetrics().cachedFragments > docsWorkerProtocol_1.DOCS_WORKER_LIMITS.maxFragments
                || this.getMetrics().cachedChars > docsWorkerProtocol_1.DOCS_WORKER_LIMITS.maxChars) {
                this.cacheByRoot.delete(this.cacheByRoot.keys().next().value);
                this.metrics.evictions += 1;
            }
            this.metrics.loads += 1;
            this.metrics.lastLoadMs = node_perf_hooks_1.performance.now() - started;
            this.logger.info(`Docs corpus loaded lazily: ${loaded.fragments.length} fragments from ${loaded.corpora.length} corpora; fingerprint=${after.value}.`);
            return cache;
        }
        throw new Error("Docs corpus changed while loading; retry the request.");
    }
    invalidate(root) {
        if (root) {
    ...
    getMetrics() {
        return {
            ...this.metrics,
            cacheRoots: this.cacheByRoot.size,
            cachedFragments: [...this.cacheByRoot.values()].reduce((sum, cache) => sum + cache.loaded.fragments.length, 0),
            cachedChars: [...this.cacheByRoot.values()].reduce((sum, cache) => sum + cache.chars, 0)
        };
    }
    beginRequest() {
    ...
    assertFresh() {
        for (const [root, fingerprint] of this.requestFingerprints) {
            if ((0, docsCorpusService_1.fingerprintDocsCorpora)(root)?.value !== fingerprint) {
                this.invalidate(root);
                throw new Error("Docs corpus changed during retrieval; retry the request.");
            }
        }
    }
    getAllowedRootCandidates() {
        ...
        candidates.push(...findAutoServerDocsRoots(this.settings.getConfigRoot(), this.environment));
        const seen = new Set();
    ...
    async loadAllowedDocsCaches(candidates) {
        ...
        for (const candidate of candidates) {
            try {
                ...
                const cache = await this.load(resolved.root);
                caches.push(cache);
                if (caches.reduce((sum, item) => sum + item.loaded.fragments.length, 0) > docsWorkerProtocol_1.DOCS_WORKER_LIMITS.maxFragments
                    || caches.reduce((sum, item) => sum + item.chars, 0) > docsWorkerProtocol_1.DOCS_WORKER_LIMITS.maxChars
                    || caches.reduce((sum, item) => sum + item.loaded.corpora.length, 0) > docsWorkerProtocol_1.DOCS_WORKER_LIMITS.maxCorpora) {
                    throw new docsWorkerProtocol_1.DocsCapacityError("Docs request exceeds the corpus budget.");
                }
            }
            catch (error) {
...
exports.DocsSearchEngine = DocsSearchEngine;
...
function searchFragments(fragments, prompt) {
    ...
    return uniqueFragments(fragments
        ...
        .sort((left, right) => right.score - left.score), (item) => item.fragment);
}
...
function selectOverviewFragmentsForPlan(fragments, preferredCorpora) {
    return uniqueFragments([...fragments]
        ...
        || left.title.localeCompare(right.title)), (item) => item)
...
function uniqueFragments(items, fragmentFor) {
    const paths = new Set();
    const urls = new Set();
    return items.filter((item) => {
        const fragment = fragmentFor(item);
        const sourceKey = JSON.stringify([fragment.sourcePath, fragment.title]);
        const urlKey = JSON.stringify([fragment.url, fragment.title]);
        const duplicate = Boolean(fragment.sourcePath && paths.has(sourceKey)) || Boolean(fragment.url && urls.has(urlKey));
        // Register even rejected items: sameFragment's path-or-URL relation is not transitive.
        if (fragment.sourcePath)
            paths.add(sourceKey);
        if (fragment.url)
            urls.add(urlKey);
        return !duplicate && Boolean(fragment.sourcePath || fragment.url);
    });
}
...
function formatDocsMetadataContext(details) {
...
function formatDocsRootMetadataLine(root) {
...
function findAutoServerDocsRoots(configRoot, environment) {
    const candidates = [];
    ...
    push(environment.serverDocs.trim(), "Документация server/docs из CODEX_ELEMENT_SERVER_DOCS");
    push(path.join(configRoot, "server", "docs"));
    let current = environment.cwd;
...
//# sourceMappingURL=docsSearchEngine.js.map