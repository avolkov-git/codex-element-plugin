...
interface PageRecord {
  ...
  description: string;
  doc_group: string;
  lsp_relevance: string;
  version: string | null;
  tags: string[];
  breadcrumbs: string[];
  search_breadcrumbs: string[];
...
async function buildRecord(args: WorkerArgs, htmlPath: string, searchByUrl: Map<string, SearchDocument>): Promise<PageRecord> {
...
async function writeIndexes(args: WorkerArgs, records: PageRecord[], searchDocuments: SearchDocument[], glossary: Array<Record<string, string>>): Promise<void> {
...
function findHtmlPages(root: string): string[] {
  const result: string[] = [];
...
function readSearchDocuments(sourceRoot: string): { documents: SearchDocument[]; byUrl: Map<string, SearchDocument> } {
...
function collectSearchDocuments(value: unknown, documents: SearchDocument[]): void {
...
function readGlossary(sourceRoot: string): Array<Record<string, string>> {
...
function slugFromHtmlPath(sourceRoot: string, htmlPath: string): string {
...
function markdownPathFromSlug(root: string, slug: string): string {
  if (slug === "_root") {
...
function htmlToMarkdown(html: string, title: string): string {
  ...
  value = value.replace(/<h([1-6])\b[^>]*>([\s\S]*?)<\/h\1>/gi, (_, level: string, content: string) => {
    return `\n\n${"#".repeat(Number(level))} ${normalizeSpace(decodeHtml(stripTags(content)))}\n\n`;
  });
  value = value.replace(/<li\b[^>]*>([\s\S]*?)<\/li>/gi, (_, content: string) => {
    return `\n- ${normalizeSpace(decodeHtml(stripTags(content)))}`;
  });
  value = value.replace(/<br\s*\/?>/gi, "\n");
  value = value.replace(/<\/(p|div|section|table|tr|ul|ol)>/gi, "\n\n");
  value = value.replace(/<img\b[^>]*alt=["']([^"']*)["'][^>]*>/gi, (_, alt: string) => `[Изображение: ${normalizeSpace(decodeHtml(alt)) || "Изображение"}]`);
  value = stripTags(value);
  value = decodeHtml(value);
  value = value.replace(/__CODE_BLOCK_(\d+)__/g, (_, rawIndex: string) => {
...
function extractHeadings(html: string): Array<{ level: number; text: string; anchor: string | null }> {
...
function extractQualifiedName(html: string): string | null {
...
function extractBaseTypes(_html: string): string[] {
  return [];
}

function inferDocGroup(slug: string, breadcrumbs: string[]): string {
  if (slug === "_root") {
    return "root";
  }
...
function inferRelevance(group: string, slug: string, breadcrumbs: string[]): string {
...
function getTags(slug: string, group: string, searchEntry: SearchDocument | undefined): string[] {
...
function buildMetadata(record: PageRecord): string {
  const lines = [
    `# ${record.title}`,
    "",
...
function buildTitlesTsv(records: PageRecord[]): string {
  return [
...
function buildKnowledgeMap(args: WorkerArgs, records: PageRecord[], glossary: Array<Record<string, string>>): string {
...
function matchFirst(value: string, regex: RegExp, group = 0): string {
...
function emitProgress(percent: number, stage: string, message: string): void {
