...
export interface HistorySearchResult { chatId: string; itemId: string; title: string; excerpt: string; createdAt: string; }

export async function searchHistory(history: PersistedChatHistory, queryValue: unknown, offsetValue: unknown = 0) {
  ...
  const items: HistorySearchResult[] = [];
  if (!query) { return { items, nextOffset: null }; }
  let matches = 0, inspected = 0;
  for (const chat of history.chats) {
    for (const item of history.transcripts[chat.id] ?? []) {
      if (++inspected % 500 === 0) { await new Promise<void>((resolve) => setImmediate(resolve)); }
      if (item.kind !== "message") { continue; }
      const position = item.text.toLocaleLowerCase().indexOf(query);
      if (position < 0 && !chat.title.toLocaleLowerCase().includes(query)) { continue; }
      if (matches++ < offset) { continue; }
      if (items.length === 50) { return { items, nextOffset: offset + items.length }; }
