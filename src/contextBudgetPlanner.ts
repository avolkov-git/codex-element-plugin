...
export interface ContextBudgetOptions {
  readonly route?: ContextBudgetRoute | string;
  readonly totalChars?: number;
  readonly minUsefulBlockChars?: number;
}

export interface ContextBudgetSourceReport {
  readonly source: ContextBlock["source"];
  readonly beforeChars: number;
  readonly afterChars: number;
  readonly beforeTokens: number;
  readonly afterTokens: number;
  readonly includedBlocks: number;
...
export interface ContextBudgetResult {
  ...
  sourceStats: string[];
  decisions: string[];
  sourceReports: ContextBudgetSourceReport[];
}

const MAX_SERVICE_CONTEXT_CHARS = 60_000;
const MIN_USEFUL_BLOCK_CHARS = 400;

const DEFAULT_SOURCE_PRIORITIES: Record<ContextBlock["source"], number> = {
  docs: 110,
  diagnostics: 95,
  project: 85,
  editorSelection: 75,
...
const ROUTE_SOURCE_PRIORITIES: Record<ContextBudgetRoute, Partial<Record<ContextBlock["source"], number>>> = {
  smallTalk: {
    editorSelection: 90,
    editorFile: 85,
    docs: 70,
...
export function applyContextBudget(blocks: readonly ContextBlock[], options: ContextBudgetOptions = {}): ContextBudgetResult {
  ...
  const sourceStats = new Map<ContextBlock["source"], {
    before: number;
    after: number;
    beforeBlocks: number;
  ...
  const selected: Array<{ block: ContextBlock; originalIndex: number }> = [];
  ...
  for (const item of ordered) {
    ...
    if (limit <= 0 || limit < minUsefulBlockChars) {
      ...
      decisions.push(
        `${source}:dropped chars=${item.block.text.length}, priority=${item.priority}, score=${formatScore(item.score)}, reason=${limit <= 0 ? "budget-exhausted" : "below-min-useful-budget"}`
      );
      continue;
    }
    ...
    if (item.block.text.length <= limit) {
      ...
      decisions.push(
        `${source}:included chars=${item.block.text.length}, priority=${item.priority}, score=${formatScore(item.score)}`
      );
      continue;
    }
    ...
    decisions.push(
      `${source}:trimmed chars=${item.block.text.length}->${trimmedText.length}, priority=${item.priority}, score=${formatScore(item.score)}, reason=budget-limit`
    );
  }
  ...
  return {
    ...
    trimmed,
    dropped,
    sourceStats: sourceReports.map((stat) =>
      `${stat.source}:${stat.beforeChars}->${stat.afterChars};tokens=${stat.beforeTokens}->${stat.afterTokens};blocks=${stat.includedBlocks}i/${stat.trimmedBlocks}t/${stat.droppedBlocks}d${stat.trimmedChars ? `,trimmed=${stat.trimmedChars}` : ""}${stat.droppedChars ? `,dropped=${stat.droppedChars}` : ""}`
    ),
    decisions,
    sourceReports
  };
...
function getEffectivePriority(block: ContextBlock, route: ContextBudgetRoute): number {
...
function normalizeRoute(route: ContextBudgetOptions["route"]): ContextBudgetRoute {
  if (
    route === "smallTalk"
    || route === "generalChat"
    || route === "docsMetadata"
...
function estimateTokens(block: ContextBlock): number {
...
function trimContextBlock(text: string, maxLength: number): string {
