import { ChatWorklogTranscriptItem, WorklogChild, WorklogOperationKind, WorklogSource, WorklogStatus } from "./types";

export interface WorklogItemPatch {
  id: string;
...
export interface WorklogOutputPatch {
  worklogId: string;
  childId: string;
}
...
interface NormalizedWorklogChildInput {
...
const MAX_CHILDREN_PER_PARENT: Partial<Record<WorklogOperationKind, number>> = {
  command: 6,
  search: 8,
  read: 8,
  file: 8,
...
export class WorklogOperationNormalizer {
  private readonly itemBindings = new Map<string, ItemBinding>();
  private readonly childCountsByParent = new Map<string, number>();
  private readonly childStatusesByParent = new Map<string, Map<string, WorklogStatus>>();
  private readonly parentBatches = new Map<string, ParentBatchState>();

  applyItemStarted(params: unknown): WorklogItemPatch | undefined {
    return this.applyItemEvent(params, "running");
  }

  applyItemCompleted(params: unknown): WorklogItemPatch | undefined {
    return this.applyItemEvent(params, "completed");
  }

  outputPatch(params: unknown): WorklogOutputPatch | undefined {
  ...
  forgetItem(params: unknown): void {
    const itemId = extractItemId(params);
    if (itemId) {
  ...
  private applyItemEvent(params: unknown, status: WorklogStatus): WorklogItemPatch | undefined {
  ...
  private resolveParentId(turnId: string, itemId: string, kind: WorklogOperationKind): string {
...
function parentStatusFor(parentId: string, statusesByParent: Map<string, Map<string, WorklogStatus>>, fallback: WorklogStatus): WorklogStatus {
...
function normalizeItem(params: unknown): NormalizedWorklogChildInput | undefined {
...
function parentWorklogBaseKey(turnId: string, itemId: string, kind: WorklogOperationKind): string {
  return `worklog-${turnId || itemId || "runtime"}-${kind}`;
}

function parentWorklogId(baseKey: string, generation: number): string {
  return generation > 0 ? `${baseKey}-${generation}` : baseKey;
}

function childWorklogId(parentId: string, itemId: string, descriptor: NormalizedWorklogChildInput): string {
...
function parentTitle(kind: WorklogOperationKind, status: WorklogStatus, count: number): string {
  const safeCount = Math.max(0, count);
  if (kind === "search") {
    if (status === "error") return "Поиск завершился с ошибкой";
    if (status === "completed") return safeCount > 1 ? `Выполнено ${safeCount} ${plural(safeCount, "поиск", "поиска", "поисков")}` : "Выполнен поиск";
    return safeCount > 1 ? `Выполняется ${safeCount} ${plural(safeCount, "поиск", "поиска", "поисков")}` : "Выполняется поиск";
  }
  if (kind === "command") {
    if (status === "error") return "Команды завершились с ошибкой";
    if (status === "completed") return safeCount > 1 ? `Выполнено ${safeCount} ${plural(safeCount, "команда", "команды", "команд")}` : "Выполнена команда";
    return safeCount > 1 ? `Выполняется ${safeCount} ${plural(safeCount, "команда", "команды", "команд")}` : "Выполняется команда";
  }
  if (kind === "file") {
    if (status === "error") return "Операции с файлами завершились с ошибкой";
    if (status === "completed") return safeCount > 1 ? `Изменено ${safeCount} ${plural(safeCount, "файл", "файла", "файлов")}` : "Изменён файл";
    return safeCount > 1 ? `Изменяется ${safeCount} ${plural(safeCount, "файл", "файла", "файлов")}` : "Изменяется файл";
  }
  if (kind === "read") {
    if (status === "completed") return safeCount > 1 ? `Изучено ${safeCount} ${plural(safeCount, "файл", "файла", "файлов")}` : "Изучен файл";
...
function humanSearchTerms(query: string): string {
...
function searchTargetLabel(query: string, searchPath: string, source: WorklogSource): string {
  if (source === "docs") {
...
function docsPathFromQuery(query: string): string {
  const match = query.match(/1cmycloud\.com\/console\/help\/(?:lang\/docs\/)?([^"\s]+)/i);
  return match?.[1] ? match[1].replace(/\/+/g, "/") : "";
}

function docsPathFromPath(value: string): string {
  const normalized = value.replace(/\\/g, "/");
  const marker = normalized.match(/(?:docs\/help\/ru|ai-docs[^/]*)\/(.+)$/i);
  return marker?.[1] ? marker[1] : "";
}

function classifySearchSource(query: string, searchPath: string, type: string, normalizedType: string): WorklogSource {
...
function classifyPathSource(value: string): WorklogSource {
...
function extractFirstNumber(value: unknown, keys: string[]): number | null {
  if (!isRecord(value)) {
    return null;
  }
  for (const key of keys) {
...
function limitText(value: string, maxLength: number): string | undefined {
  if (!value) {
    return undefined;
  }
