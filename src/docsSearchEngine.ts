...
import { DOCS_WORKER_LIMITS, DocsCapacityError } from "./docsWorkerProtocol";
...
export class DocsSearchEngine {
  ...
  private readonly metrics = { cacheHits: 0, cacheMisses: 0, evictions: 0, loads: 0, lastLoadMs: 0 };

  constructor(
    private readonly settings: Pick<SettingsService, "getDocsSettingsView" | "getConfigRoot">,
    private readonly logger: Pick<Logger, "info" | "warn">,
    private readonly environment = { cwd: process.cwd(), serverDocs: process.env.CODEX_ELEMENT_SERVER_DOCS ?? "" }
  ) {}
  ...
  private plannerInputFromCaches(caches: DocsCache[]): DocsPlannerInput {
  ...
  private async load(root: string): Promise<DocsCache> {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      const before = attempt === 0 ? fingerprint : fingerprintDocsCorpora(normalizedRoot);
      const loaded = await loadDocsCorpora(normalizedRoot, DOCS_WORKER_LIMITS);
      const after = fingerprintDocsCorpora(normalizedRoot);
      if (!before || before.value !== after?.value) {
        continue;
      }
      const chars = loaded.fragments.reduce((total, fragment) => total + docsFragmentChars(fragment), 0);
      ...
      while (this.cacheByRoot.size > DOCS_WORKER_LIMITS.maxCacheRoots
        || this.getMetrics().cachedFragments > DOCS_WORKER_LIMITS.maxFragments
        || this.getMetrics().cachedChars > DOCS_WORKER_LIMITS.maxChars) {
        this.cacheByRoot.delete(this.cacheByRoot.keys().next().value!);
        this.metrics.evictions += 1;
      }
      this.metrics.loads += 1;
      this.metrics.lastLoadMs = performance.now() - started;
  ...
  assertFresh(): void {
    for (const [root, fingerprint] of this.requestFingerprints) {
      if (fingerprintDocsCorpora(root)?.value !== fingerprint) {
  ...
  private async loadAllowedDocsCaches(candidates: readonly DocsRootCandidate[]): Promise<DocsCache[]> {
    ...
    for (const candidate of candidates) {
      try {
        ...
        const cache = await this.load(resolved.root);
        caches.push(cache);
        if (caches.reduce((sum, item) => sum + item.loaded.fragments.length, 0) > DOCS_WORKER_LIMITS.maxFragments
          || caches.reduce((sum, item) => sum + item.chars, 0) > DOCS_WORKER_LIMITS.maxChars
          || caches.reduce((sum, item) => sum + item.loaded.corpora.length, 0) > DOCS_WORKER_LIMITS.maxCorpora) {
          throw new DocsCapacityError("Docs request exceeds the corpus budget.");
        }
      } catch (error) {
        if (error instanceof DocsCapacityError) {
          throw error;
        }
...
function uniqueFragments<T>(items: T[], fragmentFor: (item: T) => DocsFragment): T[] {
  ...
  return items.filter((item) => {
    ...
    if (fragment.sourcePath) paths.add(sourceKey);
    if (fragment.url) urls.add(urlKey);
...
export function formatDocsMetadataContext(details: DocsContextDetails): string {
...
function findAutoServerDocsRoots(configRoot: string, environment: { cwd: string; serverDocs: string }): DocsRootCandidate[] {
  const candidates: DocsRootCandidate[] = [];
