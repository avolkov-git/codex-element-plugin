...
import { applyContextBudget, ContextBudgetResult } from "./contextBudgetPlanner";
...
import {
  ManagedContextToolLoopService,
  ManagedContextToolRoute,
  ManagedContextToolWorklogEntry
} from "./managedContextToolLoopService";
...
export interface ContextTurnOrchestratorRequest {
  readonly chatId: string;
  readonly chatKind: ChatKind;
  readonly prompt: string;
  readonly runMode: ChatRunMode;
  readonly cwd: string;
  readonly rulesEnabled: boolean;
  readonly explicitContextBlocks?: readonly ContextBlock[];
...
export interface ContextTurnOrchestratorResult {
  ...
  readonly projectRoute: ProjectRoute;
  readonly diagnosticsRoute: DiagnosticsRoute;
  readonly baseRulesRoute: BaseRulesRoute;
  readonly rulesRoute: RulesRoute;
  readonly explicitCount: number;
  readonly blocks: readonly ContextBlock[];
  readonly budget: ContextBudgetResult;
  readonly hasContext: boolean;
  readonly worklog: ContextTurnWorklog;
}

export interface ContextTurnWorklog {
  readonly label: string;
  readonly details: readonly string[];
  readonly entries: readonly ContextTurnWorklogEntry[];
}

export interface ContextTurnWorklogEntry {
  readonly source: ContextBlock["source"];
  readonly label: string;
...
interface ContextTurnOrchestratorOptions {
  readonly contextRouter: ContextRouterService;
  readonly baseContext: BaseContextService;
  readonly projectContext: ProjectContextService;
  readonly docsContext: DocsRetrievalLoopService;
  readonly diagnosticsContext: DiagnosticsContextService;
  readonly managedContextTools: ManagedContextToolLoopService;
  readonly nativeContextTools: NativeContextToolLoopService;
  readonly rulesContext: RulesContextService;
  readonly profiles: UserProfileService;
  readonly state: StateStore;
  readonly logger: Logger;
  readonly onDidChange: () => void;
  readonly onDidChangeChat: (chatId: string) => void;
}
...
export class ContextTurnOrchestrator {
  constructor(private readonly options: ContextTurnOrchestratorOptions) {}

  async buildTurnContext(request: ContextTurnOrchestratorRequest): Promise<ContextTurnOrchestratorResult> {
    ...
    let baseRulesRoute: BaseRulesRoute = "skip";
    let projectRoute: ProjectRoute = "skip";
    let docsRoute: DocsRoute = "skip";
    let diagnosticsRoute: DiagnosticsRoute = "skip";
    ...
    const budget = applyContextBudget(contextBlocks, { route: routing.route });
    const worklog = buildContextTurnWorklog(toolWorklog, explicitContextBlocks, budget.blocks);
    if (contextBlocks.length) {
      this.options.logger.info(
        `context budget: mode=${budget.mode}, route=${budget.route}, limit=${budget.totalCharsLimit}, ` +
  ...
  private selectMode(shouldUseContextTools: boolean): ContextTurnMode {
    if (!shouldUseContextTools) {
  ...
  private async buildToolContext(input: {
    readonly chatId: string;
    readonly prompt: string;
    readonly chatKind: ChatKind;
    readonly routing: ReturnType<ContextRouterService["decide"]>;
  ...
  private async updateProjectState(chatId: string, chatKind: ChatKind, shouldUseProjectContext: boolean): Promise<void> {
...
function normalizeDocsRoute(route: ManagedContextToolRoute): DocsRoute {
  if (route === "overview") {
    return "overview";
  }
  if (route === "added") {
...
function normalizeDiagnosticsRoute(route: ManagedContextToolRoute): DiagnosticsRoute {
  if (route === "added") {
    return "added";
  }
  if (route === "empty") {
...
function normalizeProjectRoute(route: ManagedContextToolRoute): ProjectRoute {
  return route === "added" ? "added" : "skip";
}

function buildContextTurnWorklog(
  toolEntries: readonly ManagedContextToolWorklogEntry[],
  explicitBlocks: readonly ContextBlock[],
  selectedBlocks: readonly ContextBlock[]
): ContextTurnWorklog {
  const selectedSources = new Set(selectedBlocks.map((block) => block.source));
  const entries: ContextTurnWorklogEntry[] = [];
  ...
  return {
    ...
    details: visibleEntries.map((entry) =>
      `${entry.label}; blocks=${entry.blockCount}; items=${entry.itemCount}; chars=${entry.chars}${entry.truncated ? "; truncated=true" : ""}`
    )
  };
}

function shouldShowToolWorklog(
  entry: AggregatedToolWorklogEntry,
  selectedSources: ReadonlySet<ContextBlock["source"]>
): boolean {
...
interface AggregatedToolWorklogEntry {
  readonly source: "docs" | "project" | "diagnostics";
...
function aggregateToolEntries(
  entries: readonly ManagedContextToolWorklogEntry[],
  source: AggregatedToolWorklogEntry["source"]
): AggregatedToolWorklogEntry | undefined {
...
function aggregateRoute(entries: readonly ManagedContextToolWorklogEntry[]): ManagedContextToolRoute {
...
function docsLabel(entry: AggregatedToolWorklogEntry): string {
  if (entry.status === "error") {
...
function projectLabel(entry: AggregatedToolWorklogEntry): string {
  if (entry.status === "error") {
...
function diagnosticsLabel(entry: AggregatedToolWorklogEntry): string {
  if (entry.status === "error") {
